...
class UserFetchResponse(BaseModel):
    """
    Represents the response returned when fetching a user.
...
class UserFetchAllResponse(BaseModel):
    """
    Represents the response returned when fetching all users.
    ...
    class Config:
        ...
        """
        extra = "forbid"  # Disallow extra fields in the input data.